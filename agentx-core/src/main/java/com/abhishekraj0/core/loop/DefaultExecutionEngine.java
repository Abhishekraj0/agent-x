...
public class DefaultExecutionEngine implements ExecutionEngine {
    ...
    public DefaultExecutionEngine(ChatModel model, ToolRegistry toolRegistry, List<Guardrail> guardrails, PermissionManager permissionManager, ApprovalProvider approvalProvider, com.abhishekraj0.api.event.EventBus eventBus) {
        this.model = model;
    ...
    public ExecutionResult execute(ExecutionRequest request) {
        ...
        if (eventBus != null) {
            eventBus.publish(new com.abhishekraj0.core.event.ExecutionStartedEvent(state.executionId(), agentReq.input()));
        }
        ...
        if (responseMessage.toolCalls() != null && !responseMessage.toolCalls().isEmpty()) {
            ...
            for (ToolCall toolCall : responseMessage.toolCalls()) {
                ...
                if (eventBus != null) {
                    eventBus.publish(new com.abhishekraj0.core.event.ToolCalledEvent(state.executionId(), tool.id().name(), toolCall.argumentsJson(), toolOutput));
                }
            }
            ...
            if (eventBus != null) {
                eventBus.publish(new com.abhishekraj0.core.event.ExecutionCompletedEvent(state.executionId(), responseMessage.content()));
            }
